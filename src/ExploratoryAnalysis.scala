...
object ExploratoryAnalysis extends App {
  ...
  val raw = spark.read.text("data-files\\final_project")
  ...
  val data = spark.read.option("inferSchema", true).csv("data-files\\final_project").limit(1000).toDF(columns: _*).cache()
